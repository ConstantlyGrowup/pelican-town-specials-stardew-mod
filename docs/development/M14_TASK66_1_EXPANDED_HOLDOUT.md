...
修正后同集差值为 **+23/96（+23.96 个百分点）**。修正发生在观察结果之后，因此只能作为**事后分析**并与原始主指标并列，不能回写冻结 `metrics.json`、`jev.jsonl`、Query/Gold 或声称它是预先规定的盲评结果。截至本节首次记录时，旧 24 菜的原始 14/24→18/24 仍单列，未在本报告中重新裁决或合并；后续用户对简版报告明确了新的描述性合并口径，见下方补充。两组样本都是策划题集，不能推断真实用户总体效果；产品默认仍为 LEGACY。

#### 后续人审报告的合并口径（2026-09-24）

用户明确只按菜名去重，允许不同菜复用相同原料 query。旧 24 菜与新 96 菜的中英文菜名均无重复；30 个原料 query 重叠不妨碍以菜为单位作描述性合计。旧 24 菜两侧各有 3 项 `Wheat Flour`→`246` 的同类 JEV 判否；按相同事后规则只读复算，其全菜正确为 **17/24→21/24**。加上本报告的独立 96 菜后，统一事后修正的描述性合计为 **65/120→92/120**。此数字不替代任一批冻结的原始 JEV 主结果，且共享原料 query 不应被当作完全独立的语义测试；其他合并指标与耗时见 [人审简版](M14_INGREDIENT_RAG_HUMAN_REVIEW.md)。
