...
| 字段 | 值 |
|---|---|
| overall_state | v1_5_6_release_candidate |
| project_phase | 收集品按最新收录优先排序已实现并提交；v1.5.6 本地 bundle/installer 门禁全绿，待 push 双分支与 tag |
| product_implementation_started | true |
| active_session_id | `2026-09-05-v1-5-6-release` |
| active_session_state | verification |
| active_session_type | release |
| current_task | v1.5.6 发布：push 双分支/tag → 核验 Release 三资产 → 收口 released |
| blocker | 无 |
| next_action | 用户在终端 push 双分支与 `v1.5.6` tag → 核验 release.yml 与三资产 → 收口 released |
...
| v1.5.5 发布提交 | MVP：试用迁移 `8ea63c1`、版本链 `f7a272f`、schema `6a77cf3`、RC `0079cce`、e2e 修复 `ee2e885`、收口随本提交；main：整树同步 `abb970b`、e2e 修复 `700ba03`+收口；annotated tag `v1.5.5` 指向 `ee2e885` |
...
| 最新发布 | v1.5.6（RC）；GitHub Actions run 待触发；正式 Release 仍为 v1.5.5 |
