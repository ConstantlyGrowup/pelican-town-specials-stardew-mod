...
| 字段 | 值 |
|---|---|
| overall_state | v1_5_4_release_candidate |
| Task 48（收集品批量删除）已随功能提交同步双分支；v1.5.4 本地 RC 门禁通过，待推送与 tag |
| product_implementation_started | true |
| active_session_id | `2026-09-03-v1-5-4-release` |
| active_session_state | verification |
| active_session_type | release |
| current_task | v1.5.4 发布：push 双分支/tag → 核验 Release |
| blocker | none |
| next_action | 用户在终端 push 双分支与 `v1.5.4` tag → 核验 release.yml 与三资产 → 收口 released |
...
| 最新提交 | MVP：批量删除 `6315591`、版本链 `5441c61`；main：`112476f`；tag `v1.5.4` 待创建与推送 |
...
## 进行中 v1.5.4 Release Session（verification）

- 收集品页多选后出现「打包菜单」与「批量删除」并列操作；批量删除复用单菜 tombstone DELETE 端点（级联清理不变），PixelModal 二次确认，部分失败保留选区并报错。
- MVP `6315591` / main `112476f` 已推送；frontend 全量 `200 passed`、tsc/eslint 全绿。
- 版本链 `5441c61`：repo 门禁 `52 passed`；`build_windows.ps1` 全流程绿（backend `895 passed`、OpenAPI drift、PyInstaller、EXE 图标/版本身份 1.5.4、bundle 内容门）；`build_installer.ps1` 产出 `Setup-v1.5.4.exe`。
...
| 最新发布 | v1.5.3；GitHub Actions run `33638152719` success（12m36s）；正式 Release 非 draft/pre-release；setup.exe（43,505,781 B）、portable ZIP（48,537,548 B）与 SHA256SUMS 已独立下载并逐项核验一致。 |
...
- Session：`2026-08-30-milestone-11-trial-experience-planning`；只做设计和 Task 拆解，不调用真实 Provider、不产生模型费用、不修改产品源码。
- 调研结论：官方 `GET /models/{model}` 不生成内容，但只能验证鉴权/模型可见，不能确认生成余额或 Chat/Images 真实链路；兼容中转站也不保证实现。最小 Chat query 仍增加费用且不能验证图像链路，因此不采用独立探测。
- 核心方案：每个试用 attempt 原本就需要的第一次真实 Provider 调用即为探测；调用前按 attemptId 原子预留，首次成功响应后确认消费并记录剩余次数，首次成功前失败/取消则释放并提示“本次未消耗”；首次成功后的后续失败不退还。
...
- 技术设计：`docs/architecture/PHASE_3_CANONICAL_MEMORY_TECHNICAL_DESIGN.md` v1.0；详细计划：`docs/plans/2026-08-25-milestone-9-canonical-memory.md` v1.0；MVP 技术设计/实施计划当前索引为 v2.7/v2.0，并新增 M11TD/M11IP v1.0。
...
| Task | commit | 结果 |
|---|---|---|
| 31 | `a14f128` | SQLite Canonical Registry 基础、owned icon 与迁移/损坏降级 |
| 32 | `735a7c5` | 正式 Ask Gus 归档登记、usage 与启动修复 |
...
- 发布执行与核验（全部完成）：push `feat/mvp-implementation`（33dd204..3604713，11 提交）；`git tag -a v1.2.0` → push tag；`release.yml` run **31769766198 success**（resolve-version → verify-and-build 全门禁 → create-release）；GitHub Release **v1.2.0** 已发布（title「Pelican Town Specials v1.2.0」，中文 release notes，2026-08-14 04:37 UTC）；产物逐一核验：`PelicanTownSpecials-Setup-v1.2.0.exe`（42,705,029 B）+ `PelicanTownSpecials-windows-x64-v1.2.0.zip`（47,485,062 B）+ `SHA256SUMS.txt`，API 下载后 `Get-FileHash` 与 SHA256SUMS 比对 **两个产物均一致**；分支 push 触发的 CI run 31769757384 亦 success。Session 关闭。用户可选 fresh-install 复验 v1.2.0。
