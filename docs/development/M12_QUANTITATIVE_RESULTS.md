...
| 既有 Memory 优化：平均单组整体耗时 | **190.6 s → 132.6 s（降低 30.4302203568%）** | 用户提供的 A、C、D、E、F 完整配对（n=5） |
...
统一使用同一指标中完整配对样本的总量公式。为避免把批次合计误读为单组耗时，整体耗时在结果表中只展示平均单组值；由于前后样本数相同，按总量和按均值计算出的降幅一致：

```text
降低百分比 = (未命中总量 - 命中总量) / 未命中总量 × 100%
```

| 指标 | 完整配对 | 未命中 | 命中 | 降低百分比 |
|---|---|---:|---:|---:|
| Token | A、D（n=2） | 合计 29.469K；平均 14.7345K | 合计 22.913K；平均 11.4565K | **22.2471071295%** |
| API 花费 | C、D、E、F（n=4） | 合计 $1.656；平均 $0.41400 | 合计 $1.485；平均 $0.37125 | **10.3260869565%** |
| 整体耗时（平均单组） | A、C、D、E、F（n=5） | 190.6 s（约 3 分 11 秒） | 132.6 s（约 2 分 13 秒） | **减少 58.0 s / 30.4302203568%** |
