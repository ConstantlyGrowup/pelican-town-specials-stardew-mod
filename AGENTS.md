...
2026-09-23 新增 Milestone 14：原料映射的数据归因、问题定位、本地 RAG 方案、JEV 评测、实现与同集对照，Task 61–66；计划见 `docs/plans/2026-09-23-milestone-14-ingredient-rag-evaluation.md` v1.4。Task66 同集对照为旧 14/24→RAG 18/24；用户追加 Task66.1 扩样 96 道新菜，独立同集原始 JEV 结果为旧 41/96→RAG 59/96，均经独立 detector PASS。用户随后确认原版 `Wheat Flour`/中文 `大麦粉` 的目录翻译歧义，并接受两侧统一事后修正结论：96 菜旧 48/96→RAG 71/96。原始主指标不改。用户进一步明确只要菜名不重复即可把旧 24 菜与新 96 菜作描述性合计；中英文菜名均无重复，统一事后修正后为旧 65/120→RAG 92/120，详情见人审简版报告。产品默认仍为 LEGACY；精确状态及未推送边界以 `STATUS.md` 为准。
...
Task50 四项修复已由用户授权主 Agent 接续完成，经独立 detector 封闭复审 PASS（36项测试），主 Agent 验收通过并本地提交，不推送。Task51按既有流程准备30/60/10数据与人工身份标签核对表；实际人审前停止。发布时正式Release为v1.5.4（M12 评测里程碑不触发Release），现正式版本已至v1.5.6；20张E2E照片已登记。精确状态见STATUS。
