...
2026-09-23 新增 Milestone 14：原料映射的数据归因、问题定位、本地 RAG 方案、JEV 评测、实现与同集对照，Task 61–66；计划见 `docs/plans/2026-09-23-milestone-14-ingredient-rag-evaluation.md` v1.4。Task66 同集对照为旧 14/24→RAG 18/24；用户追加 Task66.1 扩样 96 道新菜，独立同集原始 JEV 结果为旧 41/96→RAG 59/96，均经独立 detector PASS。用户随后确认原版 `Wheat Flour`/中文 `大麦粉` 的目录翻译歧义，并接受两侧统一事后修正结论旧 48/96→RAG 71/96；原始主指标不改，旧 24 菜单列。产品默认仍为 LEGACY；精确状态及未推送边界以 `STATUS.md` 为准。
